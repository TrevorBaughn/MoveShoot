...
public class Responsibility : MonoBehaviour
{
    ...
    public UnityEvent OnResponsibilityDeath = new UnityEvent(); //basically a delegate, but is a UnityEvent
    ...
    public float current
    {
        ...
        set
        {
            float currentStart = current;
            _current = Mathf.Clamp(value, 0, max);
            if(_current != currentStart) //prevent animation from slowing when going to extreme ends already
            {
    ...
    {
        current = max;
        animated = _animationSpeed;

        OnResponsibilityDeath.AddListener(Die); //adding Die as a listener
        Debug.Log("Enabled");
    }

    private void OnDisable()
    {
        OnResponsibilityDeath.RemoveListener(Die); //removing Die as a listener
        Debug.Log("Disabled");
    }
